...
import { BoardStatusLeftPanel } from './BoardStatusLeftPanel';
...
export const BoardStatus: React.FC = () => {
  ...
  return (
    <>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none px-6 py-8 md:px-10 lg:px-14">
        <div className="flex w-full max-w-[980px] items-center justify-between gap-3 md:gap-5 lg:gap-7">
          <BoardStatusLeftPanel currentTile={currentTile} players={players} />

          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
            className={`w-full max-w-[300px] md:max-w-sm rounded-[2.5rem] border-2 shadow-2xl ${themeColors.bg} ${themeColors.border} overflow-hidden shrink-0`}
          >
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="pointer-events-none flex shrink-0 flex-col items-center justify-center"
            >
              <div className="relative flex h-[150px] w-[150px] items-center justify-center md:h-[180px] md:w-[180px]">
                <div
                  className="absolute inset-[18px] rounded-full opacity-80 blur-md"
                  style={{
                    background: `radial-gradient(circle at 50% 30%, ${currentPlayer?.color ?? '#94a3b8'}55, transparent 68%)`,
                  }}
                />
