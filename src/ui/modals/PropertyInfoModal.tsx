...
export const PropertyInfoModal: React.FC = () => {
  ...
  const colorClass = property ? (groupColors[property.groupId] || 'bg-slate-400') : 'bg-slate-400';
  const rentLevels = property?.rentLevels || (property ? [property.rent] : []);
    ...
    <AnimatePresence>
      {inspectedPropertyId && property && (
        ...
        <motion.div
          ...
          className="w-full max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden border-4 border-slate-100 flex flex-col"
        ...
        >
          ...
          <div className="p-4 border-b-2 border-slate-100">
            <div className={`${colorClass} w-full py-6 px-4 rounded-lg text-white text-center border-2 border-black shadow-inner`}>
              <p className="text-[10px] font-black uppercase tracking-[0.4em] mb-1 opacity-80">Giấy chứng nhận</p>
